"""Core routines for direction-adaptive plane-wave DG experiments.
...
each element is supplied with its own set of real or complex wave vectors.
The remaining routines provide the direction parameterizations, graph-Riesz
conditioning diagnostics, local trace selection, residual evaluation, and the
nonlinear direction moves used in the numerical experiments.

The code mirrors the notation of the manuscript.  A local Trefftz atom has the
form

    exp(i q . (x-x_K)),

where ``q.q = k_K**2`` uses the bilinear (not Hermitian) dot product.  A real
``q`` gives a propagating plane wave.  A complex angle gives an evanescent
Trefftz function without requiring a separate basis family.

Implementation notes
--------------------
* Edge quadrature is used for the PWDG form and the skeleton residual.
* The exact solution object is used only to prescribe boundary data and to
  measure errors.  It is not used to choose the optimized directions.
* ``physical_skeleton_residual_vector`` returns the real least-squares residual
  whose Euclidean norm is the weighted skeleton mismatch.
* Graph-Riesz scaling is reported separately from the raw Euclidean matrix
  condition number because the two quantities measure different objects.

This file is intentionally written as a transparent research implementation.
The emphasis is on keeping the variational quantities visible in the code.
"""
...
# -----------------------------------------------------------------------------
# PWDG assembly with an elementwise plane-wave dictionary
# -----------------------------------------------------------------------------
...
class PWDGDictionary:
    ...
    no conjugation). For real q this is an ordinary plane wave
    complex q
    ...
    def sl(self,e):
        """Return the global coefficient slice belonging to element ``e``."""
        return slice(self.off[e], self.off[e+1])

    def _basis(self,e,pts):
        """Evaluate all local Trefftz atoms of element ``e`` at ``pts``."""
        qe=self.q[e]
        return np.exp(1j * (pts-self.cent[e]) @ qe.T)

    def uh(self,e,pts):
        """Evaluate the current discrete field on one element."""
        if self.nloc[e]==0:
            return np.zeros(len(pts),dtype=complex)
        return self._basis(e,pts) @ self.u[self.sl(e)]

    def grad_uh(self,e,pts):
        """Evaluate the gradient of the current discrete field on one element."""
        if self.nloc[e]==0:
    ...
    def edge(self,f):
        """Return quadrature points, weights, a reference normal, and edge length."""
    ...
    def _outward(self,e,nrm,pts):
        """Orient an edge normal outward with respect to element ``e``."""
        return nrm if nrm @ (pts.mean(axis=0)-self.cent[e]) > 0 else -nrm

    def element_area(self,e):
        """Return the area of element ``e``."""
        a,b,c=self.v[self.t[e]]
        return 0.5*abs((b[0]-a[0])*(c[1]-a[1])-(b[1]-a[1])*(c[0]-a[0]))

    def assemble(self):
        """Assemble the fixed-direction PWDG matrix and boundary-data vector.

        All terms are edge terms because every local atom satisfies the
        homogeneous Helmholtz equation inside its element.  Interior edge
        blocks contain the consistency and penalty terms.  Boundary blocks
        impose the all-Dirichlet data used by these experiments.
        """
        N=self.ndof
        ...
        def push(test,trial,block_test_trial):
            ...
            rows.append(rr)
            cols.append(cc)
            vals.append(np.asarray(block_test_trial).ravel())
            ...
            if e1>=0:
                ...
                for eA,nA in ((e0,n0),(e1,-n0)):
                    for eB,nB in ((e0,n0),(e1,-n0)):
                        if self.nloc[eA]==0 or self.nloc[eB]==0:
                            continue
    ...
    def solve(self):
        """Solve the assembled fixed-direction PWDG linear system."""
...
# -----------------------------------------------------------------------------
# Wave-vector parameterizations and standard dictionaries
# -----------------------------------------------------------------------------

def q_real(k,theta):
    """Return the real plane-wave vector ``k(cos(theta),sin(theta))``."""
    return k*np.array([np.cos(theta),np.sin(theta)],dtype=complex)


def uniform_dictionary(n, exact, m, angle_offset=0.0):
    """Construct ``m`` equally spaced real directions on every element."""
    v,t=square_mesh(n)
...
def transmission_oracle_dictionary(n, exact):
    v,t=square_mesh(n)
    cent=v[t].mean(axis=1)
    out=[]
...
# -----------------------------------------------------------------------------
# Error and conditioning diagnostics
# -----------------------------------------------------------------------------

def l2_error(s,nsub=10):
    """Compute the relative L2 error by elementwise triangle quadrature."""
...
def graph_data(A):
    """Return raw and graph-Riesz condition diagnostics for the discrete operator."""
...
def greedy_graph_select(full, initial, max_add=30, rank_tol=1e-11, candidates=None, batch=1):
    ...
    if candidates is not None:
        cand_mask[np.asarray(candidates,dtype=int)]=True
    ...
    for it in range(max_add+1):
        ...
        if len(Sarr):
            ...
            try:
                c[Sarr]=la.solve(AS,F[Sarr],assume_a='gen')
            except la.LinAlgError:
                c[Sarr]=la.lstsq(AS,F[Sarr])[0]
        full.u=c
        r=F-A@c
        # subset metrics
        ...
        cond_gr=np.nan
        if len(Sarr)>0:
            GS=G[np.ix_(Sarr,Sarr)]
            HS=(A[np.ix_(Sarr,Sarr)]+A[np.ix_(Sarr,Sarr)].conj().T)/2
            eg=la.eigvalsh((GS+GS.conj().T)/2)
            if eg.min()>1e-13*max(eg.max(),1):
                try:
                    lam=la.eigvalsh((HS+HS.conj().T)/2,(GS+GS.conj().T)/2)
                    sig=np.sqrt(1+lam*lam)
                    cond_gr=float(sig.max()/sig.min())
                except Exception:
                    pass
...
# -----------------------------------------------------------------------------
# Two-material transmission dictionaries
# -----------------------------------------------------------------------------
...
def solve_region_space(n, exact, lower_q, upper_q, nq=30):
    ...
    if s.ndof:
        try:
            ...
            try:
                s.u=la.solve(A.toarray(),F,assume_a='gen')
            except la.LinAlgError:
                s.u=la.lstsq(A.toarray(),F)[0]
    else:
...
def candidate_block_score(old, n, exact, lower_q, upper_q, region, q_candidate, nq=30, rank_tol=1e-12):
    ...
    if region=='lower':
        lq.append(np.asarray(q_candidate,dtype=complex))
    else:
        uq.append(np.asarray(q_candidate,dtype=complex))
...
def region_graph_greedy(n, exact, steps=5, allow_evan=False, coarse_deg=10.0, nq=30, verbose=False, min_novelty=1e-3):
    ...
    for it in range(steps+1):
        ...
        if allow_evan:
            ...
            if x0>0:
                bounds=(max(1.000001*k,x0-2*step),min(qmax,x0+2*step))
            else:
                bounds=(max(-qmax,x0-2*step),min(-1.000001*k,x0+2*step))
            def objx(qx):
                q=np.array([qx,1j*np.sqrt(qx*qx-k*k)],dtype=complex)
...
# -----------------------------------------------------------------------------
# Complex-angle representation and automatic direction search
# -----------------------------------------------------------------------------
...
def _automatic_complex_candidate(old, n, exact, lower_q, upper_q, region,
    ...
    for _,th0,et0,_ in scored[:
        ...
        th=float(rr.x[0]%(2*np.pi))
        et=float(np.clip(rr.x[1],-eta_max,eta_max))
        q=q_complex_angle(k,th,et)
        sc,nov=candidate_block_score(old,n,exact,lower_q,upper_q,region,q,nq=nq)
...
# -----------------------------------------------------------------------------
# Weighted physical skeleton residual and nonlinear optimization
# -----------------------------------------------------------------------------
...
def optimize_region_directions(n, exact, lower_z, upper_z, nq=20, eta_max=1.8,
    ...
    lower_z/upper_z are arrays of (theta,eta)
    no exact/physical directions enter.
    """
    from scipy.optimize import minimize
