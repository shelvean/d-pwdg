...
----------------------
The time dependence is exp(+i omega t), so outgoing cylindrical waves are
represented by Hankel functions H^(2). The local basis is exp(i q dot (x-x_K)),
and ``dirs`` stores wave-vector / spatial phase-gradient angles q.

No additional pi-rotation is applied to the initial direction set.
...
def exact_val(x, y):
    ...
    # On the L-shaped domain use theta in [0, 3pi/2] about the re-entrant
    # corner; map negative angles into [0, 2pi).
    ...
    # Old-project convention: exp(+i omega t), hence H^(2) is outgoing.
    return (
        term
        + 4 * hankel2(0, KAPPA * R1)
        + hankel2(0, KAPPA * R2)
    )


def exact_grad(x, y):
    # Analytic Bessel polar gradient plus H^(2) gradients.
    x = np.asarray(x)
    y = np.asarray(y)
    r = np.hypot(x, y)
    th = np.arctan2(y, x)
...
def longest_edge(v, T):
    E = [
        tuple(sorted((int(T[0]), int(T[1])))),
        tuple(sorted((int(T[1]), int(T[2])))),
        tuple(sorted((int(T[2]), int(T[0])))),
    ]

    return max(
        E,
        key=lambda e: np.linalg.norm(v[e[1]] - v[e[0]]),
    )


def bisect_conforming(v, t, marked, dirs):
    # Mark the longest edge of every requested triangle. Any triangle incident
    # to a marked edge is subdivided using all of its marked edges, so
    # neighboring elements share the same midpoint.
    split_edges = {
        longest_edge(v, t[int(k)])
        for k in marked
    }
    ...
    for k, T in enumerate(t):
        ...
        if len(se) == 0:
            ...
            # Two split edges share one vertex.
            e1, e2 = se
            common = list(
                set(e1).intersection(e2)
            )[0]
            ...
            m1 = midpoint[
                tuple(sorted((common, v1)))
            ]
            m2 = midpoint[
                tuple(sorted((common, v2)))
            ]
...
def q_real(theta):
    """Wave vector q(theta)=kappa(cos(theta),sin(theta))."""
            ...
            if e1 >= 0:
                ...
                for ea, na in (
                    ...
                    for eb, nb in (
                        ...
                        # Old-project exp(+i omega t) PWDG convention.
                        C = (
                            -0.5j
                            * qbn[None, :]
                            -0.5j
                ...
                # This benchmark is all-Dirichlet on the exterior boundary.
                e = e0
                n = self.outn(
                    e,
                    n0,
...
def graph_norm_exact_boundary(v, t):
    # sqrt(alpha*k ||g||^2_boundary), the only exact boundary trace term
    # for this all-Dirichlet benchmark.
    ev, et = edges(t)
...
def initial_directions():
    """Five canonical equispaced q-directions, with no pi rotation."""
    return (
        2
        * np.pi
        * np.arange(P0)
...
def hybrid_run():
    ...
    if os.path.exists(ck):
        ...
        # CORRECTION: no +pi rotation.
        base = initial_directions()
        dirs = [
            base.copy()
            for _ in range(len(t))
        ]

        norm = graph_norm_exact_boundary(
            v,
            t,
        )
    ...
    for cyc in range(
        ...
        # Stage 2: freeze the learned q-directions and continue with h-refinement.
        if (
            cyc
            >= DIRECTION_STAGE_END_CYCLE
        ):
            v, t, dirs = bisect_conforming(
                v,
                t,
                marked,
                dirs,
        ...
        for K in marked:
            ...
            if len(dirs[K]) < PMAX:
                # Candidate q-directions: also no pi rotation.
                for ang in (
                    2
                    * np.pi
                    * np.arange(6)
...
def pure_h_run(norm):
    ...
    # CORRECTION: same canonical unrotated q-set as the hybrid calculation.
    base = initial_directions()
    dirs = [
        base.copy()
        for _ in range(len(t))
