import os; os.environ["RIEMANNFEM_EQUILIBRATE"]="1"
...
if stage!='solve':
    ...
    pickle.dump(out,open(f'run/sweqblk_r{r}_L{level}_{a}_{b}.pkl','wb'));print('cells',a,b,'done',round(time.perf_counter()-t),'s')
else:
    import glob
    files=sorted(glob.glob(f'run/sweqblk_r{r}_L{level}_*.pkl'),key=lambda f:int(f.split('_')[-2]));loc={1:[],2:[]}
    for f in files:
    ...
    np.save(f'run/sweqlam_r{r}_L{level}.npy',lam)
