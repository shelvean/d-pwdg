...
from fast_bessel import bessel_j_and_derivative_integer_modes, bessel_j_and_derivative_amos_block
BESSEL_MODE=os.environ.get("BESSEL_MODE","miller").lower()
...
def basis_vals(K,rel_angles,M,x):
    ...
    E=np.exp(1j*np.outer(phi,modes))
    # Fast consecutive-order evaluation: one vectorized Miller sequence for
    # all quadrature radii; negative modes and derivatives are algebraic.
    modes_fast,Jfast,JPfast=(bessel_j_and_derivative_amos_block(M,k*rho) if BESSEL_MODE=="amos" else bessel_j_and_derivative_integer_modes(M,k*rho))
    # fast_bessel returns (mode,point); basis arrays use (point,mode).
    J=Jfast.T; JP=JPfast.T
    V=J*E
...
for p in budgets:
    ...
    pd.DataFrame(rows).to_csv(ROOT / 'data' / 'hybrid_global_dtn_results_fast.csv',index=False)
    pd.DataFrame(selrows).to_csv(ROOT / 'data' / 'hybrid_selector_candidates_fast.csv',index=False)
